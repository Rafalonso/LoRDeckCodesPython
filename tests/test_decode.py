...
from lor_deckcodes.decode import LoRDeckCodes, CardCodeAndCount
import os
...
class BaseTestDeckcode(TestCase):
    def setUp(self):
        ...
        for deck_block in deck_blocks:
            ...
            self.decks[deckcode] = LoRDeckCodes(card_list)


class TestDeckcodeDecode(BaseTestDeckcode):
    def test_data_decode(self):
        for deckcode, card_list in self.decks.items():
            self.assertListEqual(list(LoRDeckCodes.from_deckcode(deckcode)), list(card_list))
...
class TestDeckcodeEncode(BaseTestDeckcode):
    def test_data_decode(self):
        for card_list in self.decks.values():
            ...
            decoded_string = LoRDeckCodes.from_deckcode(encoded_string)
